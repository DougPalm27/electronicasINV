    ...
    const tabla = $('#tblTiposVehiculo').DataTable({
        ...
        columns: [
            ...
            { data: null, className: 'text-center', orderable: false,
              render: (d,t,r) => `
                <button class="btn btn-sm btn-warning btn-editar-tv me-1"
                        data-id="${r.id_tipo_vehiculo}" data-nombre="${esc(r.nombre)}" title="Editar">
                    <i class="bi bi-pencil"></i></button>
                <button class="btn btn-sm ${r.activo==1?'btn-danger':'btn-success'} btn-toggle-tv"
                        data-id="${r.id_tipo_vehiculo}" title="${r.activo==1?'Desactivar':'Activar'}">
